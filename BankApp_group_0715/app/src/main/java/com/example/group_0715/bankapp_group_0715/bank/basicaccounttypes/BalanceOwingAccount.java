...
public class BalanceOwingAccount extends Account {
  ...
  private static final long serialVersionUID = 323239L;
  
  /**
   * Initialize a BalanceOwingAccount object.
  ...
  public void addInterest(Context context) {
    ...
    // Calculating the interest as a percentage of the interest rate with the ABSOLUTE VALUE of the interest rate
    BigDecimal interestAddedToBalance = this.getBalance(context).multiply(interestRate);

    // Calculate the updated balance as related to their previous balance with a negative addition
    BigDecimal updatedBalance = this.getBalance(context).add(interestAddedToBalance.negate());
  ...
  public void setBalance (BigDecimal balance){
    // The balance for a balance owing must be negative

package com.example.group_0715.bankapp_group_0715.bank.database;
...
public class DatabaseDriverA extends SQLiteOpenHelper {
  ...
  protected long insertNewUserWithHashedPassword(String name, int age, String address, int roleId, String password) {
    long id = insertUser(name, age, address, roleId);
  ...
  // created by Alan
  private void insertHashedPassword(String hashedPass, int userId) {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    ContentValues contentValues = new ContentValues();
    contentValues.put("USERID", userId);
  ...
  // created by Alan
  protected int getMessageViewed(int messageId) {
    SQLiteDatabase db = getReadableDatabase();
    Cursor cursor = db.rawQuery("SELECT VIEWED FROM USERMESSAGES WHERE ID = ?",
            new String[] {String.valueOf(messageId)});
    cursor.moveToFirst();
    int result = cursor.getInt(cursor.getColumnIndex("VIEWED"));
    cursor.close();
    return result;
  }

  // created by Alan
  protected int getMessageRecipient(int messageId) {
    SQLiteDatabase db = getReadableDatabase();
    Cursor cursor = db.rawQuery("SELECT USERID FROM USERMESSAGES WHERE ID = ?",
            new String[] {String.valueOf(messageId)});
    cursor.moveToFirst();
    int result = cursor.getInt(cursor.getColumnIndex("USERID"));
    cursor.close();
    return result;
  }

  // created by Alan
  protected Cursor getAllUserAccounts() {
    SQLiteDatabase sqLiteDatabase = getReadableDatabase();
    return sqLiteDatabase.rawQuery("SELECT * FROM USERACCOUNT", null);
  ...
  protected void deleteAllRoles() {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    sqLiteDatabase.execSQL("delete from ROLES");
  }
  // created by Alan
  protected void deleteAllAccountTypes() {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    sqLiteDatabase.execSQL("delete from ACCOUNTTYPES");
  }

  // created by Alan
  protected void deleteAllPasswords() {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    sqLiteDatabase.execSQL("delete from USERPW");
  }
  ...
  // created by Alan
  protected void deleteAllUserAccounts() {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    sqLiteDatabase.execSQL("delete from USERACCOUNT");
  }

  // created by Alan
  protected void deleteAllMessages() {
    SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    sqLiteDatabase.execSQL("delete from USERMESSAGES");
  }

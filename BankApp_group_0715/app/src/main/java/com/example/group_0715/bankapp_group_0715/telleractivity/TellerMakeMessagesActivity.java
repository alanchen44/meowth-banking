...
public class TellerMakeMessagesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.teller_make_message);
    ...
    private boolean validCustomerIdCheck(String idText) {
        boolean validId = false;
        int idInt = -1;
        // verify userId could be found on db and is a customer
        ...
        try {
            ...
            int customerTypeId = rm.findRoleId("CUSTOMER", this);

            for (User cur : allUsers) {
                int curId = cur.getId();
                if (curId == idInt && cur.getRoleId() == customerTypeId) {
                    validId = true;
                }
            }
    ...
    public void makeMessage(View view){
        ...
        for (String cur : idStringArray) {
            boolean isValid = validCustomerIdCheck(cur);
            if(!isValid) {
                validId = false;
            }
        }
        ...
        if (messageTooLong) {
            ...
            successString = getResources().getString(R.string.error);
            printMessage = getResources().getString(R.string.invalidIdCustomer);
